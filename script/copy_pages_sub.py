...
def del_number_of_urls(html):
    ...
    new_html = re.sub(r'(<script +class="local" +type="text/javascript" +src=")[0-9_a-zA-Z]+/[0-9_a-zA-Z]+/[0-9]+_',r'\1js/' , new_html, flags=re.MULTILINE)
    new_html = re.sub(r'<base href="../../" />',r'<base href="../">', new_html, flags=re.MULTILINE)
    return new_html

def copy_pages(base_dir, dest_base_dir, dest_elem_dir, url):
    print(base_dir+url)
    ...
    with open(dest_base_dir+ dest_elem_dir + new_url, "w", encoding="utf-8", newline="\n") as f:
        f.write(html_rev) 
